...
class LogChatbotAdaptiveService
{
    ...
    public function exportRows(?string $idKelas = null, ?string $idLevel = null, ?string $idSoal = null): Collection
    {
        $logs = $this->buildAdaptiveLogQuery($idKelas, $idLevel, $idSoal, '')
    ...
    private function mapAdaptiveLogToHistoryItem(ChatbotAdaptiveLog $log): array
    {
        ...
        $waktuDetik = $completionProgress['waktu_detik'] ?? $this->resolveWaktuDetik($log, $detail);
        $durasiDetik = $this->resolveDurasiDetik($log, $detail, $waktuDetik);
        $jumlahLangkah = $completionProgress['jumlah_langkah'] ?? (int) ($log->jumlah_langkah ?? 0);
